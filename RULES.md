...
4. **ugType = overseas-top + GPA ≤ 3.7** → cap 到 A+（v5 批注：非美本 3.7 顶多 A+，UBC 87/100 同样适用）
5. **ugType = cn-双非 + 无大厂实习 + 无科研** → cap 到 A-
6. **ugType = cn-211 + 无科研 + 无大厂** → cap 到 A
7. **GPA < 3.3 + 无 research + 无大厂** → cap 到 B+
8. **没填 GPA** → cap 到 B（信息不足按保守处理）
9. **陆本 + 托福 < 100 或雅思 < 7** → 整体降一档（海本不触发，因为他们用英语教学）

**有美国学习经历优先用美国 GPA**（v5 批注，针对交换 / 双学位 / 暑研）：

- profile 字段 `hasUsStudyExperience=true` + `usStudyGpa` 填了 → classifier 用美国段 GPA + `usStudyGpaScale` 评估（覆盖陆本主 GPA）
- 优先级：`usStudyGpa` > `jointForeignGpa`（中外合办） > 主 `gpa`
- 适用场景：北工大 + UCB 交换；西电 + VT EIT；NC State + 川大 2+2 等
...
### SS / S / A+ 档顶档保底覆盖（v5 批注）

对 SS/S/A+ 三个档的同学，保底栏走特殊规则：

- **MSML / MSEE 类不进保底**（`topTierNoSafety: true`）—— Caltech MSEE、CMU MSML、Stanford MSEE 若原本在该档 safety 桶里，会自动移到 match
- **按 ugType 注入身份化保底**（`schoolLists.safetyOverrides`）：
  - 美本同学（`us-top` / `us-mid`）→ 注入 **UIUC MCS** 作为保底
  - 陆本 + 海本同学（非美本）→ 注入 **UW EE PMP（MSECE）+ UWB MSCS** 作为保底
- **保底总数 cap = 4**（按 barIndex 降序保留 top 4），避免转码同学保底列表过长

### v5 批注落地的禁推 / 限定规则

| 项目 | 字段 | 含义 |
|---|---|---|
| UMich MSCS（所有 3 处）/ UCSB MSCS | `dontRecommend: true` | 不再推荐，外申命中率太低 |
| Stanford MSCS | `requiresMajorBucket: ["cs","ee/ece"]` + `requiresTier: ["SSS","SS","S"]` | 仅 CS / EE 背景 + 顶档 |
| Stanford ICME | `requiresMajorBucket: ["math"]` + `requiresTier: ["SSS","SS","S"]` | 仅数学 / 统计 + 顶档 |
| Yale MSCS 2year（3 处）/ UIUC MSCS（4 处） | `requiresResearch: true` | 必须有 research 输出（domestic-paper+），否则不推 |
| Caltech MSEE、CMU MSML、Stanford MSEE | `topTierNoSafety: true` | SS/S/A+ 档不进保底，挪到 match |
